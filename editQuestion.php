...
if(!empty($_POST)){
    ...
    if(empty($err_msg)){
        update($p_id,$category_id,$question,'category_id');
        if(empty($err_msg)){
        ...
        <div class="form">
            <form action="" method="post" style="overflow:hidden;">
                ...
                <label for="" class="form__label">カテゴリー
                    <select class="form__select" name="category_id" id="">
                        ...
                        <?php foreach($category as $key => $val){ ?>
                            <option value="<?php echo $val['id'] ?>" <?php if($question['category_id'] === $val['id']) echo "selected"; ?>><?php echo $val['name']; ?></option>
                        <?php } ?>
                    </select>
                </label>
                ...
                <label for="" class="form__label">内容
                    <textarea name="question" id="" cols="30" rows="10" class="form__textarea"><?php echo $question['question']; ?></textarea>
                </label>
